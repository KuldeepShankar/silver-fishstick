...
def showmysqlupdate():
    ...
    def finddata():
        ...
        cur=db.cursor()
        xa=e1.get()
        sql="select qname,opt1,opt2,opt3,opt4,correct from mysql where Qid='%s'"%(xa)
        cur.execute(sql)
        data=cur.fetchone()
        db.commit()
    ...
    def updatedata():
        ...
        xf=e6.get()
        xg=e7.get()
        sql="update mysql set qname ='%s',opt1='%s',opt2='%s',opt3='%s',opt4='%s',correct='%s' where Qid='%s'"%(xb,xc,xd,xe,xf,xg,xa)
        cur.execute(sql)
        db.commit()
    ...
    z=Label(t,text='mysql UPDATE',fg='navyblue',font='arial,30')       
    z.place(x=300,y=10)
