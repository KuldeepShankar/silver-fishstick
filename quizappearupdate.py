...
def showquizappearupdate():
    ...
    def finddata():
        ...
        cur=db.cursor()
        xa=int(e1.get())
        sql="select courseid,name,dateofexam from quizappear where rollno=%d"%(xa)
        cur.execute(sql)
        data=cur.fetchone()
        db.commit()
    ...
    def updatedata():
        ...
        xc=e3.get()
        xd=e4.get()
        sql="update quizappear set courseid ='%s',name='%s',dateofexam='%s' where rollno=%d"%(xb,xc,xd,xa)
        cur.execute(sql)
        db.commit()
    ...
    z=Label(t,text='Quizappear',fg='navyblue',font='arial,30')       
    z.place(x=300,y=10)
    ...
    d=Label(t,text='Date of appear',font='arial,30',fg='#3D365C')
    d.place(x=50,y=320)
    e4=Entry(t,width=40)
