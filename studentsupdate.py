...
def showstudentsupdate():
    ...
    def finddata():
        ...
        cur=db.cursor()
        xa=int(e1.get())
        sql="select course_id, name, address, phone, email from students where roll_no=%d"%(xa)
        cur.execute(sql)
        data=cur.fetchone()
        db.commit()
    ...
    def updatedata():
        ...
        sql="update students set course_id ='%s',name='%s',address='%s',phone='%s',email='%s' where roll_no=%d"%(xb,xc,xd,xe,xf,xa)
        cur.execute(sql)
        db.commit()
    ...
    z=Label(t,text='Students update',fg='navyblue',font='arial,30')       
    z.place(x=300,y=10)
